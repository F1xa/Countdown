...
_viewJs.UI.BUTTON.addEventListener('click', ()=>{
    const date = _viewJs.UI.INPUT.value;
    if (!date) return;
    const { years , days , hours  } = _luxon.DateTime.now().diff(_luxon.DateTime.fromISO(date), [
        'years',
        'days',
        'hours', 
    ]).values;
    _viewJs.UI.RESULT.textContent = `Yeras: ${years}, Days: ${days}, Hours: ${Math.floor(hours)}`;
});

},{"./view.js":"de63B","luxon":"dpK6X"}],"de63B":[function(require,module,exports) {
